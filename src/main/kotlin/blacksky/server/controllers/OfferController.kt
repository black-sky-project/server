...
class OfferController(private val offerService: OfferService, private val securityService: SecurityService) {
    ...
    @PostMapping("/new")
    fun post(@RequestHeader token: String, @RequestBody dto: PostOfferDto) = offerService.create(dto).toDto()

    @DeleteMapping("/delete")
    fun delete(@RequestHeader token: String, @RequestParam id: UUID) = offerService.delete(id)
}